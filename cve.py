...
def get_cve_after_date(start_date):
    ...
    if r.json()['totalResults'] > 20:
        ...
        while page < r.json()['totalResults']:
            ...
            next_page = requests.get(f'https://services.nvd.nist.gov/rest/json/cves/1.0?startIndex={page}&pubStartDate={start_date}T00:00:00:000 UTC-05:00')
            if page==0:
...
def get_cve_between(start_date, end_date):
    ...
    r = requests.get(url="https://services.nvd.nist.gov/rest/json/cves/1.0", params=urllib.parse.urlencode(params))
    if r.json()['totalResults'] > 20:
        page = 0
        while page < r.json()['totalResults']:
            if page == 0:
            ...
            time.sleep(3)
            for cve in next_page.json().get('result').get('CVE_Items'):
                with open(f'Vulnerabilities_between_{start_date}_and_{end_date}.txt', 'a+') as f:
                    f.write(format_cve_information(next_page.json()))
    else:
        return r.json()
    return 0
...
def parse_args():
    ...
    parser.add_argument("-I", "--ID", action="store", help="Enter CVE ID in the format CVE-2021-3165.")
    parser.add_argument("-b", "--between-dates", action="store_true", help="Requires -S/--Start-Date <Start Date> "
                                                                           "and -E/--End-Date <End Date>,gets all "
                                                                           "CVEs between the start and end date")
    parser.add_argument("-S", "--Start-Date", action="store", help="Enter in the format YYYY-MM-DD")
    parser.add_argument("-E", "--End-Date", action="store", help="Enter in the format YYYY-MM-DD")
    parser.add_argument("-A", "--After-Date", action="store_true", help="Requires -S/--Start-Date <start date>. Gets"
...
if __name__ == "__main__":
    ...
    if args.between_dates:
        if not args.Start_Date and args.End_Date:
            print('[-] You must enter a start date and end date. Enter --help for example.')
        else:
