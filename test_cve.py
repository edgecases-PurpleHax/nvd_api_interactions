...
def test_get_cve_by_id(requests_mock: Mocker):
    ...
    requests_mock.get(
        f"https://services.nvd.nist.gov/rest/json/cve/1.0/{cve_id}", text=cve_data
    )
...
def test_get_all_cves(requests_mock: Mocker, tmpdir):
    ...
    requests_mock.get(
        "https://services.nvd.nist.gov/rest/json/cves/1.0", text=all_cves_response
    )
